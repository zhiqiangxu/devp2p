...
type transport interface {
	...
	ping(*enode.Node) (pong v4wire.Packet, err error)
}
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	pong, err := tab.net.ping(unwrapNode(last))

	// Also fetch record if the node replied and returned a higher sequence number.
	if wirePong, ok := pong.(*v4wire.Pong); ok && last.Seq() < wirePong.ENRSeq {
...
func (tab *Table) FindnodeByID(target enode.ID, nresults int, preferLive bool) []*Node {
	tab.mutex.Lock()
	defer tab.mutex.Unlock()
